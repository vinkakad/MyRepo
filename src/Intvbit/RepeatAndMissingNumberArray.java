...
public class RepeatAndMissingNumberArray {
    ...
    public static ArrayList<Integer> repeatedNumber(final List<Integer> A) {
        ...
        for (int i = 0; i < a.size(); i++) {
            if (a.get(Math.abs(a.get(i)) - 1) > 0) {
                int changeind = (Math.abs(a.get(i)) - 1);
                a.set(changeind, -a.get(changeind));
            } else {
                repeatedNumbers.add((Math.abs(a.get(i))));
            }
        }

        for (int i = 0; i < a.size(); i++) {
            if (a.get(i) > 0) {
                repeatedNumbers.add(i+1);
            }
        }
    ...
    public static void main(String[] args) {
        ...
        System.out.println(repeatedNumber(new ArrayList<Integer>(Arrays.asList(integer))));
